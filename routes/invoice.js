...
router.post('/', rateLimiter(5, 60000), async (req, res, next) => {
  try {
    ...
    const proofs = await db.getProofsByFarmer(farmer_id)
...
router.get('/status/:payment_hash', async (req, res) => {
  try {
    const { payment_hash } = req.params
    
    // 1. Look for a saved proof matching this payment hash
    const proof = await db.getProofByHash(payment_hash)
    ...
    // 2. Check if the invoice exists at all
    const invoice = await db.getInvoiceByHash(payment_hash)
